/*
 * SHA-512 (Fused) -- same interface and padding as the componentised
 * Sha512(nBits) but built on the register-rolling fused compression
 * Sha512compressionF. Everything else is byte-identical.
 *
 * License: MIT.
 */
pragma circom 2.0.0;

include "sha512core.circom";

template Sha512F(nBits) {
    ...
    for (i=0; i<nBlocks; i++) {
        sha512compression[i] = Sha512compressionF();
    }
