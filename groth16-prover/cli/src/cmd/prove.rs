...
pub fn run(args: Args) -> Result<(), Box<dyn Error>> {
    ...
    let (tau, alpha, beta, gamma, delta) = if let Some(pk_path) = &args.proving_key {
        ...
        if let Ok(_full_pk) = FullProvingKey::deserialize_compressed(&pk_bytes[..]) {
            eprintln!("Detected FullProvingKey format (group elements only, no scalars).");
            return Err(
                "Group-element prover is not yet implemented. \
Please use a scalar-based ProvingKey (generated by the old 'ceremony' command) \
or wait for the Phase 0 prover migration to complete."
                    .into(),
            );
        }
