...
**EdDSA-JubJub is complete.** The circuit was ported from circomlib's BabyJubJub
to JubJub (`a=-1, d=0x2a93...eb1`, embedded in BLS12-381 scalar field),
optimised from 18 112 wires to 12 601 (–31%), and validated end-to-end:
compile → witness gen → ceremony-dev → prove → verify. The sole remaining
ingredient is the Merkle verify gadget, which is straightforward composition
of the existing Poseidon hash.
