...
#
# Optional Step 4/5 (auditor reveal) can be enabled with WITH_AUDITOR=1.
#
# Usage:
#   ./f5_pipeline_e2e.sh
...
echo "========================================================================"
echo ""
echo "  Steps: 1 (predicate) → 2 (ElGamal) → 3 (F5 batch pool)"
...
# ---------------------------------------------------------------------------
# Optional Step 4/5 — Auditor reveal (viewing-key / full reveal)
# ---------------------------------------------------------------------------
...
# Print batch-verify speedup from the F5 timings if available
if [ -f "$STEP3_OUT/timings.tsv" ]; then
  VERIFY_TOTAL=$(awk '$2=="verify" {t+=$3} END {printf "%.3f", t}' "$STEP3_OUT/timings.tsv")
  BATCH_TOTAL=$(awk '$2=="batch-verify" {t+=$3} END {printf "%.3f", t}' "$STEP3_OUT/timings.tsv")
