...
pub struct Args {
    ...
    /// Right point (from --point value, file, or stdin if absent, as hex). Use "identity" for the point at infinity, or "generator" for the group generator.
    #[arg(long)]
    point: Option<String>,
}
...
pub fn run(args: Args) -> Result<(), Box<dyn Error>> {
    ...
    // Resolve left point from stdin
    let mut line = String::new();
    io::stdin().read_line(&mut line)?;
    let left_bytes = resolve_point(line.trim(), &group)?;

    // Resolve right point
    let right_bytes = if let Some(val) = args.point {
    ...
    let result =
        bls12_381_aiken_cli::group_div(&group, &left_bytes, &right_bytes).map_err(|e| e)?;
